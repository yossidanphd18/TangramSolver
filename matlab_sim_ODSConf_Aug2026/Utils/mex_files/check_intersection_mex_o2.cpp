...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    // 4. Compare and return boolean (plhs[0])
    bool is_intersecting = (overlapArea > areaThreshold);
    plhs[0] = mxCreateLogicalScalar(is_intersecting);

    // 5. Return intersection area as second output (plhs[1]), if requested
    if (nlhs > 1) {
        plhs[1] = mxCreateDoubleScalar(overlapArea);
    }
}