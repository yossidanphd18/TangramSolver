function [mask, X, Y] = getCoveredArea(valid_region, g_xmin, g_xmax, g_ymin, g_ymax, step, edges_only)
    ...
    % Find indices that are NOT NaN in either bx or by
    clean_idx = ~isnan(bx) & ~isnan(by);    
    bx = bx(clean_idx);
    by = by(clean_idx);
