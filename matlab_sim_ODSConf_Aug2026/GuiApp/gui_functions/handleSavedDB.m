function appData = handleSavedDB(appData, enable_msg_box)
    ...
    try 
        ...
        % save all Goal info to SaveDB
	    SaveDB.Goal = appData.Goal;

        % Extract the true rotations and translations, add them to SaveDB
        % for later reference tests.
        [SaveDB] = extractTrueRotsAndTrans(appData, SaveDB, true);

        % Get all BASE orientations : For each Tile_k with (flip, rotation)
        % in it base position (origin).
        [SaveDB.FlipsRotsTiles] = get_all_base_orientations(SaveDB.TilesInfo, SaveDB.Grid);

        % Keep most updated data into appData!!!
        appData.SaveDB = SaveDB;
        
        % Save the DB for later usage.
