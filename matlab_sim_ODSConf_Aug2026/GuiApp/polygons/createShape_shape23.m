function [V, G, H, Gi, Gd] = createShape_shape23(appData)
    ...
    theta_deg = 180;
    txy = scale_factor * [x8,y3] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    ...
    % Polygon 5: Square
    theta_deg = 45;
    txy = scale_factor * [x5, y2] + t_xy_common;
    ...
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x2,y0] ; [x7,y0] ; [x6,y1-1/4] ; [x8,y3] ; [x7,y4] ; ...
                        [x6+1/2,y5] ; [x6,y5] ; [x6,y3] ; [x3+1/4,y3] ; [x2,y4] ; [x0,y4] ; [x3+1/4,y1-1/4] ] + t_xy_common;
