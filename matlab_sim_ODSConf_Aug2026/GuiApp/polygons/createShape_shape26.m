function [V, G, H, Gi, Gd] = createShape_shape26(appData)
    ...
    txy = scale_factor * [7/8-1/sqrt(2),y5] + t_xy_common;
    ...
    theta_deg = 0;
    txy = scale_factor * [x4-1/2,y1] + t_xy_common;
    ...
    theta_deg = 135;
    txy = scale_factor * [x6,y6] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x0,y1] ; [x0+1,y1] ; [x7,y0] ; [x7,y2] ; [x5,y2] ; [x7,y4] ; [x6,y4] ;...
                        [x6,y6] ; [x3,y4] ; [x3,y5] ; [x8,y5] ; [x2,y3] ; [x01,y3] ] + t_xy_common;
