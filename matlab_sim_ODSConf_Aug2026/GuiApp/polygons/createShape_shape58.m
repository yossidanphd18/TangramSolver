function [V, G, H, Gi, Gd] = createShape_shape58(appData)
    ...
    theta_deg = 0;
    txy = scale_factor * [x6,y1] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    ...
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x0,y0] ; [x12,y0] ; [x11,y2] ; [x10,y1] ; [x9,y1] ; ...
                        [x6,y4] ; [x3,y1] ; [x2,y1] ; [x1,y2] ] + t_xy_common;
