function [V, G, H, Gi, Gd] = createShape_shape34(appData)
    ...
    % Polygon 5: Square
    vrtxs = scale_factor * [[0,0];[0.25*sqrt(2),0];[0.25*sqrt(2),0.25*sqrt(2)];[0,0.25*sqrt(2)]];
    theta_deg = 0;
    ...
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x0,y0] ; [x4,y0] ; [x3,y2] ; [x1,y2]] + t_xy_common;
