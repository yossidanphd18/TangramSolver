function [V, G, H, Gi, Gd] = createShape_shape60(appData)
    ...
    txy = scale_factor * [x3,y4] + t_xy_common;
    ...
    theta_deg = 180;
    txy = scale_factor * [x6,y4] + t_xy_common;
    flip_agnostic = 1;
    ...
    theta_deg = 0;
    txy = scale_factor * [x12,y4] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x3,y0] ; [x6,y4] ; [x45,y4] ; [x3,y5] ; [x12,y4] ; [x0,y4]] + t_xy_common;

    % Define the holes coordinates
    x2 = 1/2+1/4; x3 = x2 + 1/4; x4 = x3 + 1/4;
    y2 = 1/2; y3 = y2 + 1/4; y4 = y2 + 1/2;
    holeX = scale_factor * [x3, x4, x3, x2] + t_xy_common(1); 
    holeY = scale_factor * [y2, y3, y4, y3] + t_xy_common(2);
    holeShape = polyshape(holeX, holeY);    
    H{1} = holeShape;
