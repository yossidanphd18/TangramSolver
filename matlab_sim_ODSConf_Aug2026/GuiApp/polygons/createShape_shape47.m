function [V, G, H, Gi, Gd] = createShape_shape47(appData)
    ...
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x1,y0] ; [x3,y0] ; [x3,y3] ; [x0,y3] ; [x0,y0] ; [x1,y1]] + t_xy_common;

    % Define the holes coordinates
    x1 = 1/sqrt(8); x2 = x1 + 1/sqrt(8);
    y2 = 2/sqrt(8); y3 = y2 + 1/sqrt(8);
    holeX = scale_factor * [x1, x2, x1] + t_xy_common(1); 
    holeY = scale_factor * [y2, y2, y3] + t_xy_common(2);
