function [V, G, H, Gi, Gd] = createShape_shape41(appData)
    ...
    % Polygon 5: Square
    theta_deg = 0;
    txy = scale_factor * [x0, y2] + t_xy_common;
    ...
    theta_deg = 180;
    txy = scale_factor * [x2,y2] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x0,y0] ; [x3,y0] ; [x3,y3] ; [x2,y2] ; [x2,y3] ; [x0,y3]] + t_xy_common;

    % Define the holes coordinates
    x1 = 1/sqrt(2)-1/sqrt(8); x2 = 1/sqrt(2);
    y2 = 2/sqrt(8); y3 = y2 + 1/sqrt(8);
    holeX = scale_factor * [x2, x1, x1] + t_xy_common(1); 
    holeY = scale_factor * [y2, y3, y2] + t_xy_common(2);
