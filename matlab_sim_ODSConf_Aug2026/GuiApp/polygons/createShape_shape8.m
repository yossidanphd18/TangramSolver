function [V, G, H, Gi, Gd] = createShape_shape8(appData)
	...
	if(1)
		theta_deg = 135;
		txy = scale_factor * [x4-2/sqrt(8),y4] + t_xy_common;
		...
		theta_deg = -45;
		txy = scale_factor * [x4,y5] + t_xy_common;
		h = (1/4)*scale_factor; w = (1/2)*scale_factor;
		area = (2*w*h)/2;
		flip_agnostic = 0; % this tile is not flip agnostic
		V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id2, flip_agnostic, 'Parallelogram', [0.2 0.4 0.7]}; k = k + 1;
    ...
    theta_deg = -135;
    txy = scale_factor * [x3,y12] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    ...
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x4,y0] ; [x5,y2] ; [x5,y5]; [x6,y5] ; [x4,y6]; [x1,y5] ; ... 
                        [x23,y5] ; [x23-1/sqrt(8),y4] ; [x23,y4]; [x2,y3] ; [x2,y12]; [x3,y12] ;... 
                        [x4,y1]] + t_xy_common;
