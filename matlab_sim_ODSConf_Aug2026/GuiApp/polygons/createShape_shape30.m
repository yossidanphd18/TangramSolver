function [V, G, H, Gi, Gd] = createShape_shape30(appData)
	...
	if(1)
		...
		theta_deg = -45;
		txy = scale_factor * [x6,y7] + t_xy_common;
		h = (1/4)*scale_factor; w = (1/2)*scale_factor;
		area = (2*w*h)/2;
		flip_agnostic = 0; % this tile is not flip agnostic
		V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id2, flip_agnostic, 'Parallelogram', [0.2 0.4 0.7]}; k = k + 1;
    ...
    theta_deg = 135;
    txy = scale_factor * [x1,y23] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    ...
    % Polygon 5: Square
    theta_deg = 0;
    txy = scale_factor * [x0, y3] + t_xy_common;
    ...
    theta_deg = -135;
    txy = scale_factor * [x5,y1] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    ...
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x0,y1] ; [x1,y1] ; [x1,y2]; [x3,y2] ; [x4,y0]; [x4,y1] ; [x5,y1] ; [x4,y23] ;... 
                        [x4,y4] ; [x5,y4] ; [x6,y7]; [x5,y7] ; [x4,y4]; [x2,y4] ; [x3,y6] ;... 
                        [x1,y6] ; [x1,y5] ; [x0,y5]; [x0,y3] ; [x1,y3]; [x1,y23]] + t_xy_common;
