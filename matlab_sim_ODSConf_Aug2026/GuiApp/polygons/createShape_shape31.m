function [V, G, H, Gi, Gd] = createShape_shape31(appData)
    ...
    theta_deg = -135;
    txy = scale_factor * [x1+1/sqrt(8),y0] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    ...
    % Polygon 5: Square
    theta_deg = 0;
    txy = scale_factor * [x5, y5] + t_xy_common;
    ...
    theta_deg = -135;
    txy = scale_factor * [x7,y5] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    ...
    theta_deg = 135;
    txy = scale_factor * [x5,y6] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x1,y0] ; [x3,y0]; [x2,y1]; [x2,y3] ; [x2+1/sqrt(2)-1/2,y4]; [x4,y4] ; ... 
                        [x4+1/sqrt(8),y4-1/sqrt(8)] ; [x4+2/sqrt(8),y4-1/sqrt(8)] ; [x5,y4]; [x5,y5] ; [x7,y5]; [x6,y6] ; ... 
                        [x2,y6] ; [x0,y4]; [x1,y2]] + t_xy_common;
