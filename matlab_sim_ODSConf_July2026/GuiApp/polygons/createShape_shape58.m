function [V, G] = createShape_shape58(appData)
...
% end
    ...
    % Polygon 5: Square
    theta_deg = 45;
    txy = scale_factor * [x1+1/4, y2] + t_xy_common;
    ...
    theta_deg = 180;
    txy = scale_factor * [x1+3/sqrt(8),y1] + t_xy_common;
    h = (1/4)*scale_factor; w = (1/2)*scale_factor;
    area = (0.5*h*w);
    flip_agnostic = 1;
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x0,y0] ; [x7,y0] ; [x6,1/2] ; [x5,y1] ; [x3,y3] ; ...
                        [x1,y1] ; [x01,1/2] ] + t_xy_common;
