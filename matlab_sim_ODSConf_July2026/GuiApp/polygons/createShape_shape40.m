function [V, G] = createShape_shape40(appData)
...
% end
    ...
    V{k} = {base_vrtxs{k}, theta_deg, txy, area, flip_id, flip_agnostic, 'MediumTriangle', [0.2 0.8 0.6]}; k = k + 1;
     ...
     % The Goal polygon.
    G = scale_factor * [[x0,y0] ; [x3,y0] ; [x3,y2] ; [x2,y1] ; [x2,y2] ; [x3,y2] ; [x3,y3] ; ...
                        [x2,y3] ; [x2,y2] ; [x1,y2] ; [x2,y3] ; [x0,y3]] + t_xy_common;

    if(0)
        load aligned_vertices_shape_40
        G2 = aligned_vertices;
        ...
        figure;
        subplot(2,2,1);
        plot(pgon1, 'FaceColor', 'red', 'FaceAlpha', 0.2);
        axis equal;

        subplot(2,2,2);
        plot(pgon2, 'FaceColor', 'red', 'FaceAlpha', 0.2);
        axis equal;
        
        dbg = 1;

        % Bonus: Calculate properties
        %areaValue = area(pgon);
        %fprintf('The polygon area is: %.2f square pixels.\n', areaValue);
    end
