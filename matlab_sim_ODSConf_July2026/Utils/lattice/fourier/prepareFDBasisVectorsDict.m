function [TBasisDictionary, TDisqualifiedDB] = prepareFDBasisVectorsDict(ProblemSpec, TImages, TImagesFD, GP)
            ...
            for r = 1:nrots
                ...
                for m = 1:npossibles
                    ...
                    if(strcmp(GP.challenge_type,'polygons'))
                        ...
                        if((r == true_rot) && (f == true_flip) && (sum(abs(true_txy2 - t_xy))==0))                         
                            StInTrueSol.allowed_indication = 1;
                        elseif((r == true_rot) && (f == true_flip) && (sum(abs(true_txy2 - t_xy))<=2))
                        ...
                        if((r == true_rot) && (f == true_flip) && (sum(abs(true_txy - t_xy))==0))
                            StInTrueSol.allowed_indication = 1;
                        else
                            StInTrueSol = [];
                        end
    ...
    for t = 1:length(MaybeTrueSolInfo)
        ...
        if(MTSI.allowed_indication == 1)
            most_true_solution_idxs(end+1) = MTSI.var_index;
        end
        % also -1 elements will be here but no harm in later usages.
        ...
        if(most_true_solution_idxs(k) > 0)
            ...
            for t = 1:length(MaybeTrueSolInfo)
                ...
                if((MTSI.shape_id == k) && MTSI.allowed_to_use)
                    check_norm = norm(true_translations{k} - MTSI.t_xy);
